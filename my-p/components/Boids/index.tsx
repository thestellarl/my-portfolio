...
export const FloaterBackground = () => {
    ...
    if (context) {
      ...
      objectData = objectData.filter((obj) => {
        ...
        objectData.map((otherBoid) => {
          const objToOther = {
            dx: otherBoid.position.x - obj.position.x,
            dy: otherBoid.position.y - obj.position.y,
          };
          const dist = Math.sqrt(
            Math.pow(objToOther.dx, 2) + Math.pow(objToOther.dy, 2)
          );

          //Movement
          if (dist < 120 * settings.current.sightDropOff) {
            let repulsive_force = normalize({
              x: otherBoid.position.x - obj.position.x,
              y: otherBoid.position.y - obj.position.y,
            });
            obj.velocity.dx +=
              (settings.current.separationFactor * repulsive_force.x) / 15;
            obj.velocity.dy +=
              (settings.current.separationFactor * repulsive_force.y) / 15;
            //cohesion average
            num_local++;
        ...
        //Apply Cohesion
        let c = normalize({
          x: obj.position.x - localAvg.x / num_local,
          y: obj.position.y - localAvg.y / num_local,
        });
        obj.velocity.dx += (settings.current.cohesionFactor * c.x) / 4;
        obj.velocity.dy += (settings.current.cohesionFactor * c.y) / 4;

        let alignmentVector = {
          x: avgVelocity.dx / num_local,
          y: avgVelocity.dy / num_local,
        };
        obj.velocity.dx +=
          (settings.current.alignmentFactor *
            (alignmentVector.x - obj.velocity.dx)) /
          20;
        obj.velocity.dy +=
          (settings.current.alignmentFactor *
            (alignmentVector.y - obj.velocity.dy)) /
          20;

        let edgeAvoidance = {
          x: Math.tan(Math.PI * (obj.position.x / width + 1 / 2)) / 400,
          y: Math.tan(Math.PI * (obj.position.y / height + 1 / 2)) / 400,
        };
        ...
        if (settings.current.showAlignment) {
          ...
          context.moveTo(obj.position.x, obj.position.y);
          context.lineTo(
            obj.position.x + obj.velocity.dx * -40,
            obj.position.y + obj.velocity.dy * -40
          );
        ...
        if (settings.current.showVision) {
          ...
          context.arc(
            obj.position.x,
            obj.position.y,
            settings.current.sightDropOff * 120,
            0,
            2 * Math.PI
          );
        ...
        context.restore();
        return !(
          Math.abs(obj.position.x - window.innerWidth / 2) >
            window.innerWidth / 2 ||
          Math.abs(obj.position.y - window.innerHeight / 2) >
  ...
  const getBoidCanvas = useCallback(
    () => <BoidCanvas settingsRef={settings} canvasRef={canvasRef} />,
    []
  );
